...
src_configure() {
	root_tests=no
	# TODO Use some local boost_version var to be equal to "1.42.0" string?
	BOOST_PKG="$(best_version ">=dev-libs/boost-1.42.0")"
	BOOST_VER="$(get_version_component_range 1-2 "${BOOST_PKG/*boost-/}")"
	BOOST_VER="$(replace_all_version_separators _ "${BOOST_VER}")"
	BOOST_INC="/usr/include/boost-${BOOST_VER}"
	use test && (( EUID == 0 )) && root_tests=yes
	export LDFLAGS="$LDFLAGS -L/usr/lib/boost-$BOOST_VER"
	export CPPFLAGS="$CPPFLAGS -I$BOOST_INC"
	use test && (( EUID == 0 )) && root_tests=yes
